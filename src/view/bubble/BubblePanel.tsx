...
export default class BubblePanel extends Panel<any, { msgs: Array<BubbleMsg> }> {
    ...
    level2ComponentMapper: Map<Level, ClassType<{ msg: BubbleMsg }, any, any>> = new Map<Level, ClassType<any, any, any>>(
        [
            [Level.NORMAL, BubbleMsgView]
        ]
    );
        ...
        return <div style={this.style} ref={this.ref}>
            {
                this.state.msgs.map(
                    (m: BubbleMsg) =>
                        React.createElement(
                            this.level2ComponentMapper.get(m.msg.level),
                            {key: m.msg.uniqueId, msg: m}
                        )
                )
            }
        </div>;
